...
<div class="container-fluid py-4">
    <div class="row">
        <div class="col-6">
            <h4>Edit Lokasi Faskes</h4>
            <?php
            include '../config/config.php';
            $no = 1;
            $id = $_GET['id'];
            $data = mysqli_query($koneksi,"select * from info_faskes where id_faskes='$id'");
            while($d = mysqli_fetch_array($data)){
            ?>
            <form method="post" action="controller/lokasi_faskes/edit.php?id=<?php echo $d['id_faskes']; ?>">
                <div class="form-group">
                    <label for="exampleFormControlInput1">Tempat</label>
                    <input type="text" name="tempat" class="form-control" id="exampleFormControlInput1" value="<?php echo $d['tempat']; ?>">
                </div>
                <div class="form-group">
                    <label for="exampleFormControlTextarea1">Alamat</label>
                    <textarea class="form-control" name="alamat" id="exampleFormControlTextarea1" rows="3"> <?php echo $d['alamat']; ?></textarea>
                </div>
                <div class="form-group">
                    <label for="exampleFormControlTextarea1">No HP</label>
                    <input type="tel" name="no_hp" class="form-control" pattern="[0-9]+" id="exampleFormControlInput1" value="<?php echo $d['no_hp']; ?>">
                </div>
                <div class="form-group">
